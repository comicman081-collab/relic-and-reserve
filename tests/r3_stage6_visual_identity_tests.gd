...
## Stage 6B pair-keyed artifact rendering and authored-NPC visual acceptance.
##
## Every production render assertion consumes the canonical instance DTO. The
## deliberately invalid required-pair recipe probes run in isolated child
## processes so their expected push_error diagnostics cannot contaminate the
## parent suite's exit contract.

const REPORT_PATH := "res://qa/R3_STAGE6_VISUAL_IDENTITY_TESTS.json"
const TEST_SCRIPT_PATH := "res://tests/r3_stage6_visual_identity_tests.gd"
const HOSTILE_REGISTRY_FIXTURE_PATH := "res://tests/r3_stage6_hostile_registry_fixture.gd"
const EXPECTED_TEST_COUNT := 12
const UNKNOWN_PAIR_PROBE_ARG := "--stage6b-required-pair-unknown-recipe-probe"
const MISSING_PAIR_PROBE_ARG := "--stage6b-required-pair-missing-recipe-probe"
const UNKNOWN_RECIPE := "FUTURE_STORY_RECIPE"
const REQUIRED_GAUGE_PAIR := "story_artifact_16|artifact_050"
...
const INSTANCE_FIXTURES := [
	{
		"id": "freshGauge16x050",
		"artifact": {"artifactSpecId": "artifact_050", "storyArtifactId": "story_artifact_16"},
		"recipe": "GAUGE", "mesh": "res://assets/artifacts/gauge.obj"
	},
	{
		"id": "freshMusic18x035",
		"artifact": {"artifactSpecId": "artifact_035", "storyArtifactId": "story_artifact_18"},
		"recipe": "MUSIC_BOX", "mesh": "res://assets/artifacts/music_box.obj"
	},
	{
		"id": "legacyMusic18x050",
		"artifact": {"artifactSpecId": "artifact_050", "storyArtifactId": "story_artifact_18"},
		"recipe": "DEFAULT", "mesh": "res://assets/artifacts/model_05.obj"
	},
	{
		"id": "unrelatedGaugeStoryWith035",
		"artifact": {"artifactSpecId": "artifact_035", "storyArtifactId": "story_artifact_16"},
		"recipe": "DEFAULT", "mesh": "res://assets/artifacts/model_05.obj"
	},
	{
		"id": "unrelatedClockStoryWith050",
		"artifact": {"artifactSpecId": "artifact_050", "storyArtifactId": "story_artifact_17"},
		"recipe": "DEFAULT", "mesh": "res://assets/artifacts/model_05.obj"
	},
	{
		"id": "specOnly035",
		"artifact": {"artifactSpecId": "artifact_035"},
		"recipe": "DEFAULT", "mesh": "res://assets/artifacts/model_05.obj"
	},
	{
		"id": "specOnly050",
		"artifact": {"artifactSpecId": "artifact_050"},
		"recipe": "DEFAULT", "mesh": "res://assets/artifacts/model_05.obj"
	}
]
const PORTRAIT_FIXTURES := [
	{
		"case_id": "shadow_gauge", "npc_id": "hana_mire", "expression": "concerned",
		"path": "res://assets/portraits/hana_mire_concerned.svg"
	},
	{
		"case_id": "shadow_clock", "npc_id": "mara_venn", "expression": "concerned",
		"path": "res://assets/portraits/mara_venn_concerned.svg"
	},
	{
		"case_id": "shadow_music_box", "npc_id": "iris_bell", "expression": "concerned",
		"path": "res://assets/portraits/iris_bell_concerned.svg"
	}
]
...
func actual_dossier_portrait(gs: Node, registry: Node, main: Node, fixture: Dictionary) -> Dictionary:
	...
	gs.persistence_enabled = false
	gs.current_stage = 6
	gs.stage_run_state = gs.default_stage_run_state(6)
	...
	return {
		...
		"passed": not artifact.is_empty() and not npc_row.is_empty() and npc_path == expected_path \
			...
			and texture_path == expected_path \
...
func imported_portrait_pixel_contract(asset_path: String) -> Dictionary:
	...
	var left_eye: Dictionary = eye_pixel_evidence(image, Rect2i(86, 110, 38, 34)) if size == Vector2i(256, 320) else {}
	var right_eye: Dictionary = eye_pixel_evidence(image, Rect2i(132, 110, 38, 34)) if size == Vector2i(256, 320) else {}
	var total_colors := image_unique_colors(image, Rect2i(0, 0, 256, 320)) if size == Vector2i(256, 320) else 0
	var hair_colors := image_unique_colors(image, Rect2i(58, 22, 140, 92), 1) if size == Vector2i(256, 320) else 0
...
func finish(gs: Node, main: Node) -> void:
	...
	var report := {
		"suite": "R3 Stage 6B Visual Identity",
...
func run_required_pair_probe(missing_recipe: bool) -> void:
	...
	var original_value: Dictionary = canonical_registry.STORY_ARTIFACT_RENDER_OVERRIDES.get(REQUIRED_GAUGE_PAIR, {}).duplicate(true)
	...
	hostile_registry.configure_pair_override("story_artifact_16", "artifact_050", hostile_value)
	get_root().add_child(hostile_registry)
	var dto: Dictionary = hostile_registry.get_artifact_instance_render_dto({
		"artifactSpecId": "artifact_050", "storyArtifactId": "story_artifact_16"
	})
	var marker := "STAGE6B_MISSING_PAIR_RECIPE_PROBE" if missing_recipe else "STAGE6B_UNKNOWN_PAIR_RECIPE_PROBE"
	var probe := {
...
func run() -> void:
	...
	record(
		"S6B-INSTANCE-DTO-WHITELIST-01",
		"Every Stage 6 instance fixture exposes the exact ten-key public render DTO and exact nested allowlists",
		shape_failures.is_empty(),
		{"fixturesChecked": INSTANCE_FIXTURES.size(), "topKeys": TOP_LEVEL_DTO_KEYS, "failures": shape_failures}
	)

	var pristine_gauge: Dictionary = registry.get_artifact_instance_render_dto(INSTANCE_FIXTURES[0].artifact)
	var expected_gauge := pristine_gauge.duplicate(true)
	...
	pristine_gauge["specId"] = "mutated_private_spec"
	pristine_gauge["recipe"] = "MUTATED"
	pristine_gauge["palette"]["primary"] = "#000000"
	pristine_gauge["trim"]["shape"] = "hidden_truth"
	pristine_gauge["detail"]["motif"] = "auction_price"
	pristine_gauge["scale"][0] = 999.0
	var gauge_after_mutation: Dictionary = registry.get_artifact_instance_render_dto(INSTANCE_FIXTURES[0].artifact)
	...
	record(
		...
		gauge_after_mutation == expected_gauge and registry_after == registry_before and privacy_failures.is_empty(),
		{"freshDtoRestored": gauge_after_mutation == expected_gauge, "rawRegistryMutation": registry_after != registry_before, "privacyFailures": privacy_failures}
	)

	var fresh_gauge: Dictionary = registry.get_artifact_instance_render_dto(INSTANCE_FIXTURES[0].artifact)
	var fresh_music: Dictionary = registry.get_artifact_instance_render_dto(INSTANCE_FIXTURES[1].artifact)
	var gauge_mesh: Resource = load(String(fresh_gauge.get("meshPath", "")))
	var music_mesh: Resource = load(String(fresh_music.get("meshPath", "")))
	record(
		"S6B-FRESH-PAIR-MAPPING-03",
		"Fresh story 16 plus Artifact 050 maps only to GAUGE/gauge.obj and fresh story 18 plus Artifact 035 maps only to MUSIC_BOX/music_box.obj",
		String(fresh_gauge.get("specId", "")) == "artifact_050" \
			and String(fresh_gauge.get("recipe", "")) == "GAUGE" \
			and String(fresh_gauge.get("meshPath", "")) == "res://assets/artifacts/gauge.obj" \
			and gauge_mesh is Mesh \
			and String(fresh_music.get("specId", "")) == "artifact_035" \
			and String(fresh_music.get("recipe", "")) == "MUSIC_BOX" \
			and String(fresh_music.get("meshPath", "")) == "res://assets/artifacts/music_box.obj" \
			and music_mesh is Mesh,
		{"freshGauge": {"recipe": fresh_gauge.get("recipe", ""), "mesh": fresh_gauge.get("meshPath", ""), "loadable": gauge_mesh is Mesh}, "freshMusic": {"recipe": fresh_music.get("recipe", ""), "mesh": fresh_music.get("meshPath", ""), "loadable": music_mesh is Mesh}}
	)
	...
	for fixture_index: int in range(2, INSTANCE_FIXTURES.size()):
		...
		var row_ok := String(before.get("recipe", "")) == "DEFAULT" \
			and String(before.get("meshPath", "")) == "res://assets/artifacts/model_05.obj" \
			and before == after
	...
	record(
		"S6B-LEGACY-DEFAULT-ISOLATION-04",
		"Legacy story 18 plus Artifact 050 and unrelated/spec-only 035/050 remain DEFAULT on canonical model_05 with cross-contamination zero",
	...
	var unknown_probe := run_child_probe(UNKNOWN_PAIR_PROBE_ARG, "STAGE6B_UNKNOWN_PAIR_RECIPE_PROBE=", "r3_stage6b_unknown_pair_recipe_probe.log")
	var missing_probe := run_child_probe(MISSING_PAIR_PROBE_ARG, "STAGE6B_MISSING_PAIR_RECIPE_PROBE=", "r3_stage6b_missing_pair_recipe_probe.log")
	...
	record(
		"S6B-REQUIRED-PAIR-FAIL-CLOSED-05",
		"A required story/spec pair with an unknown or missing recipe returns an empty DTO instead of silently using DEFAULT",
		probes_pass,
		{"unknownRecipe": unknown_probe, "missingRecipe": missing_probe}
	)

	var gauge_artifact: Dictionary = gs.new_artifact("artifact_050", 6050, "stage6b_fresh_gauge")
	...
	var gauge_nodes := {
		"ticks": named_nodes(main.workpiece_root, "GaugeScaleTick_*").size(),
		"needle": named_nodes(main.workpiece_root, "GaugePressureNeedle").size(),
		"hub": named_nodes(main.workpiece_root, "GaugePressureHub").size(),
		"plate": named_nodes(main.workpiece_root, "GaugeCalibrationPlate").size(),
		"seal": named_nodes(main.workpiece_root, "GaugeCalibrationSeal").size(),
		"genericTrim": named_nodes(main.workpiece_root, "VariantTrim_*").size(),
		"typewriter": named_nodes(main.workpiece_root, "Cipher*").size(),
		"compass": named_nodes(main.workpiece_root, "*Compass*").size(),
		"musicBox": named_nodes(main.workpiece_root, "MusicBox*").size()
	}
	var expected_gauge_nodes := {"ticks": 13, "needle": 1, "hub": 1, "plate": 1, "seal": 1, "genericTrim": 0, "typewriter": 0, "compass": 0, "musicBox": 0}
	record(
		"S6B-GAUGE-RENDERER-06",
		"Main.tscn renders the fresh gauge with thirteen ticks, needle, hub, plate and seal and no generic trim, typewriter, compass or music-box residue",
		gauge_live_path == "res://assets/artifacts/gauge.obj" and gauge_nodes == expected_gauge_nodes \
			and not gauge_live_path.to_lower().contains("compass"),
		{"liveMeshPath": gauge_live_path, "nodes": gauge_nodes, "expected": expected_gauge_nodes}
	)

	var music_artifact: Dictionary = gs.new_artifact("artifact_035", 6035, "stage6b_fresh_music_box")
	...
	var music_nodes := {
		"cylinder": named_nodes(main.workpiece_root, "MusicBoxPinnedCylinder").size(),
		"pins": named_nodes(main.workpiece_root, "MusicBoxCylinderPin_*").size(),
		"comb": named_nodes(main.workpiece_root, "MusicBoxComb").size(),
		"teeth": named_nodes(main.workpiece_root, "MusicBoxCombTooth_*").size(),
		"stem": named_nodes(main.workpiece_root, "MusicBoxWindingStem").size(),
		"key": named_nodes(main.workpiece_root, "MusicBoxWindingKey").size(),
		"inlay": named_nodes(main.workpiece_root, "MusicBoxLidInlay").size(),
		"genericTrim": named_nodes(main.workpiece_root, "VariantTrim_*").size(),
		"typewriter": named_nodes(main.workpiece_root, "Cipher*").size(),
		"compass": named_nodes(main.workpiece_root, "*Compass*").size(),
		"gauge": named_nodes(main.workpiece_root, "Gauge*").size()
	}
	var expected_music_nodes := {"cylinder": 1, "pins": 8, "comb": 1, "teeth": 7, "stem": 1, "key": 1, "inlay": 1, "genericTrim": 0, "typewriter": 0, "compass": 0, "gauge": 0}
	record(
		"S6B-MUSIC-BOX-RENDERER-07",
		"Main.tscn renders the fresh music box with pinned cylinder, comb, seven teeth, winding stem/key and inlay and no generic trim, typewriter, compass or gauge residue",
		music_live_path == "res://assets/artifacts/music_box.obj" and music_nodes == expected_music_nodes \
			and not music_live_path.to_lower().contains("compass"),
		{"liveMeshPath": music_live_path, "nodes": music_nodes, "expected": expected_music_nodes}
	)
	...
	for fixture_value: Variant in PORTRAIT_FIXTURES:
		...
		var markers_ok := raw_mapping == expected_path and public_mapping == expected_path \
			...
			and svg_has_two_complete_eyes(svg)
		svg_rows.append({"npc": npc_id, "rawRegistryPath": raw_mapping, "publicRegistryPath": public_mapping, "twoCompleteEyes": svg_has_two_complete_eyes(svg), "passed": markers_ok})
		if not markers_ok:
			svg_failures.append(npc_id)
	record(
		"S6B-PORTRAIT-SVG-V2-08",
		"Hana, Mara and Iris resolve through explicit registry mappings to local SVG V2 busts with two complete sclera/iris/pupil/highlight eyes",
		svg_rows.size() == 3 and svg_failures.is_empty(),
	...
	record(
		"S6B-PORTRAIT-IMPORTED-PIXELS-09",
		"Each loaded portrait texture matches the current source import and contains two pixel-visible white, chromatic and dark eye systems plus detailed hair/outfit/prop color zones",
		imported_pixel_rows.size() == 3 and imported_pixel_failures.is_empty(),
	...
	record(
		"S6B-PORTRAIT-DOSSIER-10",
		"Each Stage 6 authored NPC source is discovered and renders one textured CaseNpcSourcePortrait at least 96 by 120 with generic fallback zero",
		dossier_results.size() == 3 and dossier_failures.is_empty(),
		{"cases": dossier_results, "failures": dossier_failures.size()}
	)

	var signature_035: Variant = registry.visual_signature("artifact_035")
	var signature_050: Variant = registry.visual_signature("artifact_050")
	var artifact_035: Dictionary = gs.new_artifact("artifact_035", 9635, "stage6b_signature_035")
	var artifact_050: Dictionary = gs.new_artifact("artifact_050", 9650, "stage6b_signature_050")
	var stored_035: Variant = artifact_035.get("visualSignature", null)
	var stored_050: Variant = artifact_050.get("visualSignature", null)
	record(
		"S6B-VISUAL-SIGNATURE-CONTAINMENT-11",
		"The legacy visual_signature and newly stored signatures for both reused specs remain unchanged String values",
		typeof(signature_035) == TYPE_STRING and typeof(signature_050) == TYPE_STRING \
			and typeof(stored_035) == TYPE_STRING and typeof(stored_050) == TYPE_STRING \
			and not String(signature_035).is_empty() and not String(signature_050).is_empty() \
			and stored_035 == signature_035 and stored_050 == signature_050,
		{"registry035Type": type_string(typeof(signature_035)), "registry050Type": type_string(typeof(signature_050)), "stored035Type": type_string(typeof(stored_035)), "stored050Type": type_string(typeof(stored_050)), "equal035": stored_035 == signature_035, "equal050": stored_050 == signature_050, "schemaChangeAttempted": false}
	)
	...
	record(
		"S6B-SPEC-ONLY-DISTRIBUTION-12",
		"The eighty-spec distribution remains seventy-eight DEFAULT, one cipher and one sextant while the pair-only recipe allowlist includes later visual identities",
